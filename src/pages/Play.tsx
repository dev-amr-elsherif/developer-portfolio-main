...
// Piece SVG components (اختصاراً لم نغيرها لأنها تعمل)
...
const Play = () => {
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.error || data.details || 'Failed to connect to Sherif');
      }
    ...
    } catch (error: any) { // حلينا مشكلة unknown هنا
      console.error('Chat error:', error);
      const errorMessage: ChatMessage = {
        role: 'assistant',
        content: `Oops! ${error.message || 'Something went wrong'}. Try again? 😅`
      };
